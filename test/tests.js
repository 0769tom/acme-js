...
var acme = require("../index.js");

describe("Acme JS", function () {
	...
	it("should have properties", function () {
		...
		properties.forEach(function (property) {
			(acme).should.have.property(property);
		});
	});

	describe("crypto.js", function () {
		var crypto = acme.crypto;
	...
	describe("dapp.js", function () {
		...
		describe("#createDapp", function () {
			...
			var options = {
				category: 0,
				name: "Acme Guestbook",
				description: "The official Acme guestbook",
			...
			describe("returned dapp", function () {
				var keys = acme.crypto.getKeys("secret");
				var secondKeys = acme.crypto.getKeys("secret 2");
				...
				it("should be signed correctly", function () {
					var result = acme.crypto.verify(trs);
					(result).should.be.ok;
				});
				...
				it("should be second signed correctly", function () {
					trs.amount = 0;
					var result = acme.crypto.verifySecondSignature(trs, secondKeys.publicKey);
					(result).should.be.ok;
				});
	...
	describe("delegate.js", function () {
		...
		describe("#createDelegate", function () {
			...
			describe("returned delegate", function () {
				...
				it("should be signed correctly", function () {
					var result = acme.crypto.verify(trs, keys.publicKey);
					(result).should.be.ok;
				});
				...
				it("should not be second signed correctly now", function () {
					trs.amount = 100;
					var result = acme.crypto.verify(trs, secondKeys.publicKey);
					(result).should.be.not.ok;
				});
	...
	describe("transaction.js", function () {
		var transaction = acme.transaction;
		...
		describe("#createTransaction with second secret", function () {
			...
			var keys = acme.crypto.getKeys(secondSecret);
			...
			describe("returned transaction", function () {
				...
				it("should be second signed correctly", function () {
					var result = acme.crypto.verifySecondSignature(trs, keys.publicKey);
					(result).should.be.ok;
				});
	...
	describe("vote.js", function () {
		...
		describe("#createVote", function () {
			var createVote = vote.createVote,
				vt = null,
				publicKey = acme.crypto.getKeys("secret").publicKey,
			...
			describe("returned vote", function () {
				...
				it("should be signed correctly", function () {
					var result = acme.crypto.verify(vt);
					(result).should.be.ok;
				});

				it("should be second signed correctly", function () {
					var result = acme.crypto.verifySecondSignature(vt, acme.crypto.getKeys("second secret").publicKey);
					(result).should.be.ok;
				});
